...
class HwOBSTest extends TestCase
{
    ...
    {
        // 加载配置
        $config = TestHelper::loadConfig('HwOBS');
        if (! $config) {
            $this->markTestSkipped('华为云OBS配置不存在');
        }

        // 创建真实的HwOBS实例
        $this->obs = new HwOBS($config);

        // 准备测试数据
        $this->testKey      = 'test_' . uniqid() . '.txt';
        $this->testFilePath = TestHelper::createTestFile('This is a test file for Huawei OBS.');
    }
